...
export default function FormsWorkspace() {
  ...
  async function load() {
    ...
    setTemplates(data.templates); setCustomers(data.customers); setForms(data.forms);
  ...
  const visibleForms=forms.filter(form=>filter==="all" ? true : filter==="active" ? form.status!=="archived" : form.status===filter);
  ...
  return <section className="forms-workspace">
    <header className="forms-heading"><div><p>CLIENT OPERATIONS</p><h1>Forms & approvals</h1><span>Create, send, complete, approve, and retain every customer document in one place.</span></div><button onClick={()=>setShowCreate(current=>!current)}>{showCreate?"Cancel":"+ New form"}</button></header>
    {showCreate&&<form className="forms-create" onSubmit={createForm}>
      ...
      <label>Customer<select name="leadId" required defaultValue=""><option value="">Choose a customer…</option>{customers.map(item=><option key={item.id} value={item.id}>{item.business} — {item.name}</option>)}</select></label>
